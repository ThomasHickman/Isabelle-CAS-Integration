...
subsection \<open> Thomas' extra rules: \<close>

lemma has_vector_derivative_sinh[derivative_intros]:
  fixes f :: "real \<Rightarrow> real"    
  assumes f: "(f has_vector_derivative f') (at x within s)"
  shows "((\<lambda>x. sinh(f x)) has_vector_derivative (f' * cosh(f x))) (at x within s)"
  using assms by (simp add: sinh_def cosh_def, prove_vec_deriv)

lemma has_vector_derivative_cosh[derivative_intros]:
  fixes f :: "real \<Rightarrow> real"    
  assumes f: "(f has_vector_derivative f') (at x within s)"
  shows "((\<lambda>x. cosh(f x)) has_vector_derivative (f' * sinh(f x))) (at x within s)"
  using assms by (simp add: sinh_def cosh_def, prove_vec_deriv)

lemma has_vector_derivative_tanh[derivative_intros]:
  fixes f :: "real \<Rightarrow> real"    
  assumes f: "(f has_vector_derivative f') (at (x::real) within s)"
  shows "((\<lambda>(x::real). tanh(f x)) has_vector_derivative (f' * (1 - (tanh (f x))^2))) (at (x::real) within s)"
  oops
thm has_field_derivative_tanh

lemma has_vector_derivative_arcsin[derivative_intros]:
  fixes f :: "real \<Rightarrow> real"    
  assumes f: "(f has_vector_derivative f') (at x within s)"
  assumes "-1 < f x & f x < 1"
  shows "((\<lambda>x. arcsin(f x)) has_vector_derivative (f' * inverse (sqrt(1-(f x)^2)))) (at x within s)"
  using assms by prove_vec_deriv

lemma has_vector_derivative_arccos[derivative_intros]:
  fixes f :: "real \<Rightarrow> real"    
  assumes f: "(f has_vector_derivative f') (at x within s)"
  assumes "-1 < f x & f x < 1"
  shows "((\<lambda>x. arccos(f x)) has_vector_derivative (f' * inverse (- sqrt(1-(f x)^2)))) (at x within s)"
  using assms by prove_vec_deriv

lemma has_vector_derivative_arctan[derivative_intros]:
  fixes f :: "real \<Rightarrow> real"    
  assumes f: "(f has_vector_derivative f') (at x within s)"
  shows "((\<lambda>x. arctan(f x)) has_vector_derivative (f' * inverse (1 + (f x)^2))) (at x within s)"
  using assms by prove_vec_deriv
